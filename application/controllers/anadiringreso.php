...
class Anadiringreso extends CI_Controller {
    ...
    function index(){
        ...
        //Si existe añado los datos a la base de datos, tengo que validar los campos del formulario
            ...
            if(isset($_POST['enviar'])){ 
                ...
                $this->form_validation->set_rules('tipoingreso', 'Tipo de Ingreso', 'required|xss_clean');
                $this->form_validation->set_rules('formapago', 'Forma de Pago', 'callback_formapago_check');
                $this->form_validation->set_rules('tipoingreso', 'Tipo de Ingreso', 'callback_tipoingreso_check');
    ...
    public function tipoingreso_check($tipoingreso) {
        if($tipoingreso==0){
            $this->form_validation->set_message('tipoingreso_check','Tienes que seleccionar un tipo de ingreso');
            return FALSE;
        }
        else{
            return TRUE;
